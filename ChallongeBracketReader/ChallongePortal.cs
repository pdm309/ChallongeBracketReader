...
namespace ChallongeBracketReader
{
    class ChallongePortal
    {
        ...
        public void ReportMatchWinner(int tournamentId, int matchId, int winnerId, int[] scores)
        {
            var request = new RestRequest(string.Format("tournaments/{0}/matches/{1}.xml", tournamentId, matchId), Method.GET);
            request.AddParameter("api_key", ApiKey);
            request.AddParameter("tournament", tournamentId);
            request.AddParameter("match", matchId);

            //scores = new SetScore[] { new SetScore { Player1Score = 0, Player2Score = 0 } };
            ...
            requestScores.AddParameter("tournament", tournamentId);
            requestScores.AddParameter("match", matchId);
            requestScores.AddParameter("attachment", score);
